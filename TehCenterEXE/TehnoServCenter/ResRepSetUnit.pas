...
uses BoldHandlesDataModule, DateUtils, BoldModelDataModule, ReportsDataModuleUnit, MainFormUnit, BusinessClasses;
 ...
 if BoldHandlesDM.blhAllResources.Count>0 then
  if (BoldHandlesDM.blhAllResources.CurrentElement as TTip_tovara).VseFlag then
    begin
      ...
      ReportsDM.blhResRepList.Expression:='Rashod_po_operacii.allInstances';
    end
  else
    begin
      ReportsDM.blhResRepList.Expression:='';
...
begin
  ReportsDM.blhResRepList.Expression:=ReportsDM.blhResRepList.Expression+'->select(vhodit_kak_poz_rashoda.rashoduetsya_dlya_remonta.dataVydachi>=#'+FormatDateTime('YYYY-MM-DD',DateTimePicker1.Date)+')->select(vhodit_kak_poz_rashoda.rashoduetsya_dlya_remonta.dataVydachi<=#'+FormatDateTime('YYYY-MM-DD',DateTimePicker2.Date)+')';
  Close;
...
begin
 ...
 ShowMessage('Отсутствуют позиции расхода материалов по заданным параметрам!');
