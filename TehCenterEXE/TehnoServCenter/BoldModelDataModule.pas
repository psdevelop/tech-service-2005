...
begin
  ...
  MainForm.MainStatusBar.Panels[0].Text:='Нет соединения';
...
begin
  ...
  MainForm.MainStatusBar.Panels[0].Text:='Соединение активно';
  ...
  if BoldHandlesDM.blhAllSotr.Count=0 then
    begin
      ...
      (BoldHandlesDM.blhAllSotr.CurrentElement as TSotrudnik_remont).Familiya:='Все';
  ...
  if BoldHandlesDM.blhAllSotr.Count=0 then
    begin
      ...
      (BoldHandlesDM.blhAllResources.CurrentElement as TTip_tovara).Naimenovanie:='Все';
      BoldModelDM.BoldUpdateDBAction1.Execute;
    end;
  BoldHandlesDM.blhAllResources.Expression:='Tip_tovara.allInstances';
  ...
  {if (ProgrammSettingsForm.blhPrSett.CurrentElement as TTekushie_znacheniya).Startovaya_data_uch<StrToDate('15.01.2006') then
    begin
      ...
      MainForm.MainStatusBar.Panels[1].Text:='Ограниченная версия, осталось запусков - '+IntToStr(DaysBetween(StrToDate('15.10.2006'),(ProgrammSettingsForm.blhPrSett.CurrentElement as TTekushie_znacheniya).Startovaya_data_uch));
      BoldModelDM.BoldUpdateDBAction1.Execute;
    end
  else
    begin
      ...
      if ((ProgrammSettingsForm.blhPrSett.CurrentElement as TTekushie_znacheniya).Startovaya_data_uch>=StrToDate('15.10.2006')) then
        begin
          ShowMessage('Время работы ограниченной версии истекло');
          MainForm.MainStatusBar.Panels[1].Text:='Время работы ограниченной версии истекло';
    ...
    begin
      try
        ...
        if (UserName=IniFile.ReadString('BUSINNESS_DATA','director_login','director')) or (UserName=IniFile.ReadString('BUSINNESS_DATA','buh_login','buh'))  then
          begin
            ShowMessage('Вы вошли в привелегированном режиме!');
            MainForm.NJobAccountRep.Enabled:=True;
            MainForm.NResOutRep.Enabled:=True;
            MainForm.NCheckRep.Enabled:=True;
          end;
      except
        ShowMessage('Неудачный запрос имени текущего пользователя у сервера!');
      end;
    end
  else
    begin
...
begin
  ...
  BoldPersistenceHandleFileXML1.FileName:=GetCurrentDir+'\DB_DATA.xml';
  //ADOConnection.Open();
end;
