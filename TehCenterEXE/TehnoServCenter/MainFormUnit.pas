...
uses
  ...
  ResRepSetUnit, DB, DBGrids, ShellAPI, BoldSubscription, BoldElements,
...
begin
 ...
 begin
  if IniFile.ReadString('BUSINNESS_DATA','show_CDS_act_err','NO')='YES' then
   ShowMessage('Неудачная активация компонентов поставки данных! Ошибка: '+E.Message);
 end;
 end;
end;
...
begin
 ...
 ShowMessage('Нет текущего акта работ!');
...
begin
 ...
 //   end
 // else
 //   ShowMessage('Невозможно осуществить операцию!');
end;
...
begin
 if (BoldHandlesDM.blhAllSotr.CurrentElement as TSotrudnik_remont).VseFlag then
 begin
   ShowMessage('Выберите технического сотрудника!');
 end
 else if BoldHandlesDM.blhAllSotr.Count=0 then
 begin
   ShowMessage('Добавьте хотя бы одного сотрудника технического сотрудника!');
 end
 else
 begin
  BoldHandlesDM.BoldListHandleAddNewActionAllCust.Execute;
  ...
  ShowMessage('Добавьте в справочник хотя бы одного сотрудника!');
 if BoldHandlesDM.blhActContent.Count>0 then
  BoldHandlesDM.blhSelForSotrActs.CurrentIndex:=BoldHandlesDM.blhSelForSotrActs.List.IndexOf((BoldHandlesDM.blhActContent.CurrentElement as TSostav_priemki_vydachi).Pozicii_uslug_dlya);
end;
...
begin
if MessageDlg('Удалить акт?',mtConfirmation,mbYesNo,0)=mrYes then
...
begin
 if MessageDlg('Удалить заказ?',mtConfirmation,mbYesNo,0)=mrYes then
...
begin
  BoldHandlesDM.blhConsForRes.CurrentBoldObject.Delete;
end;
